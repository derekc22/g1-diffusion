...
from models.stage2_task_space_diffusion import Stage2MLPModel, Stage2TransformerModel
...
def load_latest_checkpoint(model_dir: str) -> str:
    ckpts = sorted(glob.glob(os.path.join(model_dir, "checkpoints", "stage2_epoch_*.pt")))
    if not ckpts:
...
def main():
    ...
    if architecture == "mlp":
        model = Stage2MLPModel(state_dim=state_dim, cond_dim=model_cond_dim).to(device)
    else:
        model = Stage2TransformerModel(state_dim=state_dim, cond_dim=model_cond_dim).to(device)
    ...
    for i in range(B):
        ...
        seq_for_name = meta_i.get("seq_name") or f"stage2_seq_{i:03d}"
