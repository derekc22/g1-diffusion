"""
Stage 2 Flow Matching Training Script: Hand Positions → Full-Body Motion

Trains the Stage 2 flow matching model which generates full-body robot poses
conditioned on (rectified) hand positions.
...
def main():
    yml = load_config(os.path.join(PROJECT_ROOT, "config", "train_stage2_fm.yaml"))
    train_yml = yml["train"]
    ...
    exp_name = f"stage2_fm_e{num_epochs}_b{batch_size}_lr{lr}_w{window_size}_s{stride}_{architecture}_"
    ...
    # Create model
    print(f"\nCreating Stage 2 FM {architecture} model")
    ...
    for epoch in range(num_epochs):
        ...
        for step, batch in enumerate(dataloader):
            # Get data
            state = batch["state"].to(device)    # (B, T, state_dim) - normalized
            cond = batch["cond"].to(device)      # (B, T, cond_dim) - hand positions
            ...
            # 3. Interpolate: x_t = (1-t)*state + t*x0
            x_t = schedule.interpolate(state, x0, t)

            # 4. Velocity target: v = x0 - state
        ...
        if (epoch + 1) % save_every == 0 or epoch == num_epochs - 1:
            ckpt_file = os.path.join(ckpt_path, f"stage2_fm_epoch_{str(epoch).zfill(6)}.pt")
            torch.save({
        ...
        if epoch % save_every == 0 or epoch == num_epochs - 1:
            ...
            plt.xlabel("epoch")
            plt.ylabel("loss")
            plt.title("Stage 2 Flow Matching Loss")
            plt.grid(True)
