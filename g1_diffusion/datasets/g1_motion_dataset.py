...
class G1MotionDataset(Dataset):
    """
    Dataset over GMR-retargeted Unitree G1 motion files.
    ...
    def __init__(
        ...
        use_root_rot_6d: bool = True,
        use_dof_pos: bool = True,
        preload: bool = False,
    ):
        super().__init__()
        ...
        self.use_root_pos = use_root_pos
        self.use_root_rot_6d = use_root_rot_6d
        self.use_dof_pos = use_dof_pos
        self.preload = preload

        file_paths = sorted(glob.glob(os.path.join(root_dir, pattern)))
        if not file_paths:
            raise RuntimeError(f"No motion files found in {root_dir} with pattern {pattern}")
    ...
    def _load_file(self, file_idx: int, path: str) -> Dict[str, Any]:
        ...
        object_rot_6d = data.get("object_rot_6d")
        ...
        data_proc = {
            ...
            "root_rot": root_rot,
            "dof_pos": dof_pos,
            "local_body_pos": np.asarray(data.get("local_body_pos", None))
                if "local_body_pos" in data else None,
