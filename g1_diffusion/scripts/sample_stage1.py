...
from models.stage1_task_space_diffusion import Stage1TaskSpaceModel, Stage1TransformerModel
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional explicit checkpoint path; defaults to latest in save_dir.",
    )
    parser.add_argument(
        "--backbone",
        type=str,
    ...
    ckpt_path = args.ckpt_path if args.ckpt_path else load_latest_checkpoint(args.save_dir)
    ckpt = torch.load(ckpt_path, map_location=device)
    ckpt_config = ckpt.get("config", {}) or {}
    ckpt_backbone = ckpt_config.get("backbone", args.backbone)
    if ckpt_backbone != args.backbone:
        print(
            f"Warning: checkpoint backbone '{ckpt_backbone}' differs from requested '{args.backbone}'. Using checkpoint backbone."
        )
    ...
    if backbone == "mlp":
        model = Stage1TaskSpaceModel(state_dim=state_dim, cond_dim=model_cond_dim).to(device)
    else:
    ...
    for i in range(B):
        ...
        motion_data = {
            ...
            "object_rot_6d": obj_rot6d.astype(np.float32),
